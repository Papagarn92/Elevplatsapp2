...
const CLASSROOM_CONFIG = {
    "Sal 302": { // Denna sal slumpar bara nummer.
        max_seats: 24,
        columns_per_row: 8,
        gang_column_width: "100px"
    },
    "Linus Sal 302": { // Denna sal använder din klasslista.
        max_seats: 24,
        columns_per_row: 8,
        gang_column_width: "100px",
        allows_names: true
...
function renderDesks() {
    ...
    if (config.layout_map) { config.layout_map.forEach(renderDesk); }
    else {
        let rowCounter = 1; let columnCounter = 1;
        for (let i = 0; i < max; i++) {
            const deskNumber = max - i; 
...
function initializeSession() {
    ...
    lastDrawnPair = savedData.lastDrawnPair || null; // Ladda föregående dragning
    ...
    // KORRIGERAD LOGIK: Visa senast dragna nummer, annars '?'
    if (lastDrawnPair) {
...
function assignAllAtOnce() {
    if (confirm('Är du säker? Detta skapar en helt ny slumpmässig placering för alla elever.')) {
        ...
        lastDrawnPair = null; // Ingen enskild dragning att visa
...
function drawOneSeat() {
    ...
    resultDisplay.textContent = drawnNumber; // Uppdatera displayen direkt
    lastDrawnPair = { name: null, seat: drawnNumber };
    
    saveData({ assignments, lastDrawnPair }); // Spara den nya dragningen
    
    // Uppdatera bara det som behövs, anropa INTE initializeSession()
    renderDesks();
    updateUI();
}
